...
uint256 hashGenesisBlock("0x00000000692c5ed1da2f480cd8d6aa6aa6b16deff49c26bf5a773120328b8b12");
...
static unsigned int GetNextWorkRequiredV1(const CBlockIndex* pindexLast, const CBlockHeader *pblock, int algo)
{
    ...
    if (pindexLast == NULL)
		...
		int64_t lTargetTimespan = fPastFork ? nTargetTimespan : nTargetTimespanORIG;
		int64_t lTargetSpacing = fPastFork ? nTargetSpacing : nTargetSpacingORIG;
		int64_t lInterval = fPastFork ? nInterval : nIntervalORIG;		
		
    // Only change once per interval
    if ((pindexLast->nHeight+1) % lInterval != 0)
    {
        if (TestNet())
        {
            ...
            if (pblock->nTime > pindexLast->nTime + lTargetSpacing*2)
                return nProofOfWorkLimit;
            else
            {
                // Return the last non-special-min-difficulty-rules-block
                const CBlockIndex* pindex = pindexLast;
                while (pindex->pprev && pindex->nHeight % lInterval != 0 && pindex->nBits == nProofOfWorkLimit)
                    pindex = pindex->pprev;
                return pindex->nBits;
            }
        }
    ...
    for (int i = 0; pindexFirst && i < lInterval-1; i++)
